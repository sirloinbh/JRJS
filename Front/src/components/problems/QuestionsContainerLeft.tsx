...
const QuestionsContainerLeft: React.FC = () => {
  ...
  const questionTopNumber = 4 * (page.pageNumber - 2) + 1;
  const questionBottomNumber = 4 * (page.pageNumber - 2) + 2;

  // 현재 페이지 번호에 따라 난이도를 계산합니다.
  const difficulty = String(page.pageNumber - 1);

  // 질문의 설명을 가져오기 위한 계산
  const questionTopText = data && data[difficulty] && data[difficulty].find(q => q.questionnumber === 1)?.description || '';
  const questionBottomText = data && data[difficulty] && data[difficulty].find(q => q.questionnumber === 2)?.description || '';
